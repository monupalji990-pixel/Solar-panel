...
export default function AddTaskLogic(props) {
  ...
  useEffect(() => {
    if (startLoader && props.isLoadingData) {
  ...
  }, [startLoader, props.isLoadingData]);

  const getComplainTitle = () => {
    if (props.consumer?.fullName) {
      return 'COMPLAIN - ' + props.consumer?.fullName;
    } else if (props?.company?.company) {
      return 'COMPLAIN - ' + props?.company?.company;
    } else return "";
  };

  const initialValues: any = {
    title: getComplainTitle(),
    priority: null,
    description: "",
