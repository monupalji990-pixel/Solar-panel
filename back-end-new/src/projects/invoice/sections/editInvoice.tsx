...
export default function EditItem(props) {
  return (
    <MyDrawer
      drawerSize="90vw"
      iconName="Invoice"
      open={props.open == "editInvoiceDrawer"}
...
function EditItemLogic(props) {
  ...
  const _editInvoice = (payload) =>
    dispatch(invoiceAdminAction.editInvoice(payload));
  ...
  const _getInvoiceDetails = (payload) =>
    dispatch(invoiceAdminAction.getInvoiceDetails(payload));
  ...
  useEffect(() => {
    ...
    _isLoadingData(true);
    _getInvoiceDetails({ _id: props.user?._id });
  }, []);

  useEffect(() => {
    if (invoiceState.invoiceDetails) {
      setRadioToggle(invoiceState.invoiceDetails?.company ? "2" : "1");
      setCompanySelected(invoiceState.invoiceDetails?.fromCompany || "");
    }
  }, [invoiceState.invoiceDetails]);
  ...
  const initialValues = {
    Company: invoiceState.invoiceDetails?.company?.businessName
      ? {
        label: invoiceState.invoiceDetails?.company?.businessName,
        value: invoiceState.invoiceDetails?.company?._id,
      }
      : null,
    Consumer: invoiceState.invoiceDetails?.consumer?.firstName
      ? {
        label: invoiceState.invoiceDetails?.consumer?.firstName,
        value: invoiceState.invoiceDetails?.consumer?._id,
      }
      : null,
    lineItems:
      invoiceState.invoiceDetails?.lineItems.length > 0
        ? invoiceState.invoiceDetails.lineItems
        : [
          {
            itemId: null,
            title: "",
    ...
    dueDate: invoiceState.invoiceDetails?.dueDate
      ? new Date(invoiceState.invoiceDetails?.dueDate)
      : null,
    paymentDue: invoiceState.invoiceDetails?.paymentDue
      ? new Date(invoiceState.invoiceDetails?.paymentDue)
      : null,
    discount: invoiceState.invoiceDetails?.discount || 0,
    discountOption: invoiceState.invoiceDetails?.discountType
      ? {
        label: invoiceState.invoiceDetails?.discountType,
        value: invoiceState.invoiceDetails?.discountType,
      }
      : null,
    totalAmount: invoiceState.invoiceDetails?.totalAmount || null,
    tax: invoiceState.invoiceDetails?.tax || null,
  };
                    ...
                    <TextField
                      ...
                      onChange={(e) => {
                        ...
                        if (values.discountOption.value === "Percentage") {
                          ...
                          handleChange({ target: { name: 'discount', value: cappedValue } });
                        } else {
                          handleChange({ target: { name: 'discount', value: value } });
                        }
                      }}
                    />
                ...
                <Table>
                  <TableBody>
                    ...
                    <TableRow>
                      ...
                      <TableCell align="right">{getTotalTax20(values)}</TableCell>
                    </TableRow>

                    <TableRow>
                      <TableCell>VAT 5%</TableCell>
                      <TableCell align="right">{getTotalTax5(values)}</TableCell>
                    </TableRow>
