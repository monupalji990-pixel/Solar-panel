...
export default function assignee(props) {
    ...
    if (props.assigneeList && props.assigneeList.length > 0) {
        contactList = props.assigneeList.map(e => ({ label: e.name, value: e._id }));
    }
            ...
            <Formik
                ...
                onSubmit={(value, actions) => {
                    ...
                    if (a.CompanyID === undefined) {
                        actions.setErrors({
                            Assignee: 'Please create a company first'
                        })
                    } else {
                        actions.setErrors({})
                        setStartLoader(true);
                        ...
                        <form onSubmit={handleSubmit}>
                            <Grid container spacing={3}>
                                <Grid item xs={12} md={6}>
                                    <Select
                                        className={errors.Assignee && touched.Assignee ? 'ErrorColor' : ''}
                                        id="Assignee"
